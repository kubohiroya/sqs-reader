...
public class GenericSourceConfig {
	 ...
	 */
	public boolean match(String filename, int pageIndex) {
	 ...
	 * @param pageIndex starts with 1
	 */
	private boolean match(Argument arg, int pageIndex) {
		if (arg instanceof RangeArgument) {
			...
			if (rarg.getStart() != null) {
				...
				if (rarg.getEnd() != null) {
					...
					return (start <= pageIndex) && (pageIndex <= end);
				} else {
			...
			return (Integer.parseInt(arg.getValue()) == pageIndex);
		}
	}

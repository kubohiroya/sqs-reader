...
import net.sqs2.omr.session.traverse.MarkReadStatus.Select1Status;
import net.sqs2.omr.session.traverse.MarkReadStatus.SelectStatus;


public class MarkReadStatusRetrieveModule implements SpreadSheetTraverseEventListener{
	...
	protected MarkReadStatusRetrieveModule() {
	}
	...
	public void endRow(RowEvent rowEvent) {
		...
		//MultiHashMap<PageID, OMRProcessorErrorModel> taskErrorModelMap = rowEvent.getTaskErrorModelMultiHashMap();
		int numPageIDs = rowEvent.getNumPageIDs();
		if (0 < numPageIDs) {
			this.totalMarkReadStatus.numErrorPages += numPageIDs;
		}

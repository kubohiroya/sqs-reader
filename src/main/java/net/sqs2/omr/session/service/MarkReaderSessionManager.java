...
public class MarkReaderSessionManager{
	...
	public static synchronized MarkReaderSession create(File sourceDirectoryRoot, boolean enableSearchPageMasterFromAncestorDirectory, RemoteTaskTracker<OMRPageTask, SessionSourceServerDispatcher> taskTracker, 
			SessionSourceServerDispatcher dispatcher) throws IOException{
		MarkReaderSession session = new MarkReaderSession(sourceDirectoryRoot, enableSearchPageMasterFromAncestorDirectory, taskTracker, dispatcher);
		sessionNameBag.add(sourceDirectoryRoot);
		sourceDirectoryRootToSessionMap.put(sourceDirectoryRoot, session);
		MarkReaderSessionManager.add(session);
		return session;
	}
	...
	public static synchronized void remove(File sourceDirectoryRoot) {
		...
		sessionIDToSessionMap.remove(session.getSessionID());
	}

...
public abstract class RemoteTaskTracker<T extends OMRPageTask, D extends ServerDispatcher>{

	private int rmiPort;
	private String rmiBindingName;

	private RemoteSessionDiscoveryService<T,D> remoteSessionDiscoveryService;
	private RemoteExecutorManager<T,D> remoteExecutorManager;
	private MulticastNetworkConnection multicastNetworkConnection;

	public RemoteTaskTracker(int rmiPort, String rmiBindingName) {
		super();
		...
		try {
			...
			Logger.getLogger(this.getClass().getName()).info("MulticastGroup:Port = "+getSessionServiceMulticastAddress()+":"+getSessionServiceMulticastPort());
		} catch (IOException e) {
			if (e instanceof SocketException) {
				Logger.getLogger(this.getClass().getName()).warning("multicast disabled:"+((SocketException) e).getMessage());
				//e.printStackTrace();
			} else {
				Logger.getLogger(this.getClass().getName()).warning("multicast disabled.");
				e.printStackTrace();
			}
		}
	}
	...
	public abstract RemoteExecutorManager<T,D> createRemoteExecutorManager();

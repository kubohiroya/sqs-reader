...
import net.sqs2.omr.session.init.ConfigFileUtil;
...
public class MarkRecognitionConfigurationDialog extends JDialog {
	...
	private static boolean userDefaultConfigurationEnabled = false;
	...
	public static boolean isUserDefaultConfigurationEnabled(){
		return userDefaultConfigurationEnabled;
	}
	...
	private int getDensityThresholdValue() {
		...
		try {
			...
			Logger.getLogger(getClass().getName())
					.severe("file=" + this.configFile+ ", density=null, set default value");
		}
		return defaultValue;
	}
	...
	private int getDoubleMarkIgnoranceThresholdValue() {
		...
		try {
			...
			Logger.getLogger(getClass().getName()).severe("file=" + this.configFile+ ", recognitionMargin=null, set default value");
		}
		return defaultValue;
	}
	...
	private int getNoMarkRecoveryThresholdValue(){
		...
		try {
			...
			Logger.getLogger(getClass().getName()).severe("file=" + this.configFile+ ", noMarkRecovery=null, set default value");
		}
		return defaultValue;
	}
	...
	void storeConfiguration(int densityValue255, int doubleMarkIgnoranceValue255, int noMarkRecoveryValue255) throws IOException {
		MarkRecognitionConfigurationDialog.userDefaultConfigurationEnabled = true;
		...
		if (!this.configFile.exists()) {
			ConfigFileUtil.createConfigFile(this.configFile);
		}

...
public class XMLFormMasterFactory implements FormMasterFactory {
	...
	private void setMasterMetadata(Element svgNode, FormMaster master) throws TransformerException, XPathExpressionException {
		...
		setWidthHeight(svgNode, master);
		setDeskewGuideCenterPoints(metadataNode, master);
		Logger.getLogger(getClass().getSimpleName()).info("master schema version: "+master.getVersion());
	...
	private void setWidthHeight(Element svgNode, FormMaster master) throws TransformerException, XPathExpressionException {

		String width = this.xpathSelector.selectAttribute(svgNode, "width", SQSNamespaces.SVG_URI);
		String height = this.xpathSelector.selectAttribute(svgNode, "height", SQSNamespaces.SVG_URI);
		master.setWidth(Double.parseDouble(width));
		master.setHeight(Double.parseDouble(height));
	}
	...
	private void setMasterMaster(Element metadataNode, FormMaster master) throws TransformerException, XPathExpressionException {
		...
		master.setNumPages(Integer.parseInt(this.xpathSelector.selectAttribute(masterNode, "numPages", SQSNamespaces.SQS2007MASTER_URI)));

		String horizontalOffset = this.xpathSelector.selectAttribute(masterNode, "horizontalOffset", SQSNamespaces.SQS2007MASTER_URI);
		if (horizontalOffset != null) {
			try {
				master.setHorizontalOffset(Integer.parseInt(this.xpathSelector.selectAttribute(masterNode, "horizontalOffset", SQSNamespaces.SQS2007MASTER_URI)));
			} catch (Exception ignore) {
			}
		}
		String verticalOffset = this.xpathSelector.selectAttribute(masterNode, "verticalOffset", SQSNamespaces.SQS2007MASTER_URI);
		if (verticalOffset != null) {
			try {
				master.setVerticalOffset(Integer.parseInt(this.xpathSelector.selectAttribute(masterNode, "verticalOffset", SQSNamespaces.SQS2007MASTER_URI)));
			} catch (Exception ignore) {
			}
		}
		if(master.getVersion().equals("1.3")){
			master.setHorizontalOffset(3);
		}
		if(master.getVersion().equals("2.1.0")){
			master.setVerticalOffset(-16);
			master.setHorizontalOffset(2);
		}
	}
	...
	static ByteArrayInputStream hotfixXFormsNSURItoSVGNSURI(InputStream sqmInputStream) throws UnsupportedEncodingException, IOException {
		...
		while ((line = reader.readLine()) != null) {
			...
			if (0 <= c) {
				...
				if (DEBUG) {
					System.err.print(line.substring(0, c));
					System.err.print(replace);
					System.err.println(line.substring(c + pattern.length()));
				}

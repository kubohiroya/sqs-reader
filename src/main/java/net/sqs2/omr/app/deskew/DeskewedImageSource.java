...
public class DeskewedImageSource extends ImageTranslationFilter {
	final static float SUBPIXEL_SAMPLING_FACTOR = 2.0f;
	...
	static {
		AffineTransform transform = AffineTransform.getScaleInstance(1 / SUBPIXEL_SAMPLING_FACTOR, 1 / SUBPIXEL_SAMPLING_FACTOR);
		operator = new AffineTransformOp(transform, new RenderingHints(RenderingHints.KEY_INTERPOLATION,
				RenderingHints.VALUE_INTERPOLATION_BILINEAR));
	}

	public DeskewedImageSource(BufferedImage image, Point2D[] deskewMasterPoints, Point2D[] deskewGuidePoints, int leftValue, int rightValue, int defaultBackgroundColor) {
		this(image, deskewMasterPoints, deskewGuidePoints, defaultBackgroundColor);
	}

	public DeskewedImageSource(BufferedImage image, Point2D[] deskewMasterPoints, Point2D[] deskewGuidePoints, int defaultBackgroundColor) {
		super(image, deskewMasterPoints, deskewGuidePoints, defaultBackgroundColor);
	}
	...
	public BufferedImage cropImage(float x, float y, float w, float h) {
		BufferedImage image = new BufferedImage((int) (w * SUBPIXEL_SAMPLING_FACTOR),
				(int) (h * SUBPIXEL_SAMPLING_FACTOR), BufferedImage.TYPE_INT_RGB);
		WritableRaster raster = image.getRaster();
		Point2D.Float p = new Point2D.Float();
		int[] rgbarr = new int[3];
		for (int j = (int) (h * SUBPIXEL_SAMPLING_FACTOR) - 1; 0 <= j; j--) {
			for (int i = (int) (w * SUBPIXEL_SAMPLING_FACTOR) - 1; 0 <= i; i--) {
				try {
					ImageUtil.rgb2arr(getRGB(x + i / SUBPIXEL_SAMPLING_FACTOR, y + j / SUBPIXEL_SAMPLING_FACTOR, p),
							rgbarr);
					raster.setPixel(i, j, rgbarr);
				} catch (ArrayIndexOutOfBoundsException ex) {
					throw createException(image, x,  i, y, j, SUBPIXEL_SAMPLING_FACTOR);
				}
			}
		}
	 ...
	 */
	public int getGrayscaleDensity(Rectangle2D rect, float resolutionScaleFactor) {
		int total = 0;
		Point2D p = new Point2D.Float();
		float factor = SUBPIXEL_SAMPLING_FACTOR * resolutionScaleFactor;
		int w = (int) (rect.getWidth() * factor) - 1;
		int h = (int) (rect.getHeight() * factor) - 1;
		int numOfIgnorablePixel = 0;
		for (int i = w; 0 <= i; i--) {
			for (int j = h; 0 <= j; j--) {
				try {
					total += ImageUtil.rgb2gray(getRGB( (int)(rect.getX() + i / factor), (int)(rect.getY() + j / factor), p));
				} catch (ArrayIndexOutOfBoundsException ex) {
					//Logger.getLogger(getClass().getName()).info("OutOfFrame: width="+ image.getWidth()+", height="+ image.getHeight() +
					//		", x="+( rect.getX() + i / factor)+ ", y="+(rect.getY() + j / factor));
					numOfIgnorablePixel++;
				}
			}
		}
		return (total / ((w + 1) * (h + 1) - numOfIgnorablePixel));
	}

	/**
	 * @return grayscale value (0-255)
	 */
	public int calcMarkAreaDensityWithHorizontalSlices(Rectangle rect, float densityThreshold, float resolutionScaleFactor) {
		int total = 0;
		int height = 0;
		Point2D p = new Point2D.Float();
		...
		for (int j = h; 0 <= j; j--) {
			...
			for (int i = w; 0 <= i; i--) {
				try {
					...
					throw createException(image, rect.getX(), i, rect.getY(), j, factor);
				}
			}
	 ...
	 * @return grayscale value (0-255)
	 */
	public int calcMarkAreaDensityWithVerticalSlices(Rectangle rect, float densityThreshold, float resolutionScaleFactor) {
		int total = 0;
		int width = 0;
		Point2D p = new Point2D.Float();
		float factor = resolutionScaleFactor;
		...
		for (int i = w; 0 <= i; i--) {
			...
			for (int j = h; 0 <= j; j--) {
				try {
					...
					throw createException(this.image, rect.getX(), i, rect.getY(), j, factor);
				}
			}
	...
	public int calcMarkAreaDensity(Rectangle2D rect) {
		if(rect == null){
			return WHITE_VALUE;
		}
	 ...
	 * @return
	 */
	public int calcConvolution5x3AverageMarkAreaDensityWithDebugOut(Rectangle rect, float densityThreshold, float resolutionScaleFactor) {
	 ...
	 * @return
	 */
	public int calcConvolution5x3AverageMarkAreaDensity(Rectangle rect, float densityThreshold, float resolutionScaleFactor) {
	...
	private int[] createSamplingBitmapOfMarkArea(Rectangle rect, int w, int h) {
		Point2D p = new Point2D.Float();
		int[] src = new int[w * h];
		for (int y = h - 1; 0 <= y; y--) {
			int base = y * w;
			...
			for (int x = w - 1; 0 <= x; x--) {
				try {
					...
					throw createException(image, rect.getX(), 0, rect.getY(), 0, 1);
				}
			}
		}
		return src;
	...
	private IllegalArgumentException createException(BufferedImage image, double x, int i, double y, int j, float factor){
		return new IllegalArgumentException("width="+ image.getWidth()+", height="+ image.getHeight() +", x="+(x + i / factor)+", y="+(y + j / factor));
	}

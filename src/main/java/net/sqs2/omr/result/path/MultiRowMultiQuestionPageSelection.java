...
public class MultiRowMultiQuestionPageSelection extends MultiRowMultiQuestionSelection {
	...
	public MultiRowMultiQuestionPageSelection(MultiRowMultiQuestionPagePath multiRowMultiQuestionPagePath) throws ObjectStoreException{
		this(multiRowMultiQuestionPagePath, NUM_MAX_ITEMS_WITHIN_A_PAGE);
	}
	
	public MultiRowMultiQuestionPageSelection(MultiRowMultiQuestionPagePath multiRowMultiQuestionPagePath, int numMaxItems) throws ObjectStoreException{
	...
	private ArrayList<Answer> forwardSearch(RowAccessor rowAccessor, int numMaxItems)throws ObjectStoreException{
		// forward search
		return forwardSearch(rowAccessor, new ArrayList<Answer>(), numMaxItems);	
	}
	
	private ArrayList<Answer> forwardSearch(RowAccessor rowAccessor, ArrayList<Answer> answerList, int numMaxItems)throws ObjectStoreException{
		// forward search
		int numFocused = 0;
	...
	private ArrayList<Answer> backwardSearch(RowAccessor rowAccessor, int numMaxItems)throws ObjectStoreException{
		// forward search
		return backwardSearch(rowAccessor, new ArrayList<Answer>(), numMaxItems);	
	}
	
	private ArrayList<Answer> backwardSearch(RowAccessor rowAccessor, ArrayList<Answer> answerList, int numMaxItems)throws ObjectStoreException{
		// backward search
		int numFocused = 0;

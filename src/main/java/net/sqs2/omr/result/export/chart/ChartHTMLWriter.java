...
import net.sqs2.omr.session.traverse.MarkReadStatusRetriever.MarkReadStatus;
...
class ChartHTMLWriter extends HTMLWriter {
	private final MarkReadStatus markReadStatus;
	...
	ChartHTMLWriter(long sessionID, File chartDirectoryFile, String skin) throws IOException {
		super(skin);
		this.markReadStatus = MarkReadStatusRetriever.getMarkReadStatus(sessionID);
	...
	void drawBarChartImage(List<FormArea> formAreaList, SpreadSheet spreadSheet) throws IOException, TemplateException {
		...
		if (chartImageFile.exists() && this.markReadStatus.getTargetLastModifiedArray()[columnIndex] < chartImageFile.lastModified()) {
			return;
		}

		int[] numAnswers = this.markReadStatus.getValueTotalMatrix()[columnIndex];
	...
	void drawPieChartImage(List<FormArea> formAreaList, SpreadSheet spreadSheet) throws IOException, TemplateException {
		...
		if (chartImageFile.exists()
				&& this.markReadStatus.getTargetLastModifiedArray()[columnIndex] < chartImageFile.lastModified()) {
			return;
		}

		int[] numAnswers = this.markReadStatus.getValueTotalMatrix()[columnIndex];
		int numNoAnswers = this.markReadStatus.getNumNoValues()[columnIndex];
		int numMultipleAnswers = this.markReadStatus.getNumMultipleValues()[columnIndex];
	...
	void writeChartIndexHTMLFile(File chartDirectoryFile, SpreadSheet spreadSheet, boolean isExportChartImageMode) throws IOException, TemplateException {
		...
		int numRows = sourceDirectory.getNumPageIDsRecursive() / numPages;
